...
financeRoutes
  .route("/api/finance/quote/:stockSymbol")
  .get(async function (req, res) {
    const result = await yahooFinance.quote(req.params.stockSymbol);
    res.json(result);
  });

financeRoutes.route("/api/finance/trending").get(async function (req, res) {
  const queryOptions = { count: 5, lang: "en-US" };
  const result = await yahooFinance.trendingSymbols("CA", queryOptions);
  res.json(result);
});

financeRoutes.route("/api/finance/recommended/").get(async function (req, res) {
